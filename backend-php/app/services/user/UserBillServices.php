...
class UserBillServices extends BaseServices
{
    ...
    public function getIntegralRank() {

        /*
        SELECT 
            eub.id, 
            SUM(eub.number) as sum_integral,
            eu.avatar,
            eu.nickname
        FROM eb_user_bill eub
        LEFT JOIN eb_user eu 
        ON eub.uid = eu.uid 
        WHERE 
            eub.category="integral" 
        GROUP BY eub.uid
        ORDER BY sum_integral DESC;
        */
        $data = Db::query('SELECT eub.id, SUM(eub.number) as sum_integral, eu.avatar, eu.nickname FROM eb_user_bill eub LEFT JOIN eb_user eu ON eub.uid = eu.uid WHERE eub.category="integral" GROUP BY eub.uid ORDER BY sum_integral DESC;');
        return $data;
    }
