...
class UserBillController
{
    ...
    public function integral_rank(Request $request) {
        $data = $this->services->getIntegralRank();
        return app('json')->success($data);
    }
    ...
    public function product_list(Request $request) {
        ...
        // $data = Db::query("SELECT id, image, store_name FROM eb_store_product WHERE FIND_IN_SET(id, '" . $productIdsStr . "')");
        $data = Db::query("SELECT cpr.product_id AS id, esp.image, esp.store_name FROM custom_product_rank cpr LEFT JOIN eb_store_product esp ON esp.id = cpr.product_id WHERE cpr.enable = 1 ORDER BY cpr.sort DESC;");
        return app('json')->success($data);
    }

    public function product_user_rank(Request $request) {
        // SELECT
        //     espl.id,
        //     espl.uid,
        //     espl.product_id,
        //     SUM(espl.pay_num) AS sum_pay_num,
        //     eu.nickname,
        //     eu.avatar
        // FROM eb_store_product_log espl
        // LEFT JOIN eb_user eu ON eu.uid = espl.uid
        // WHERE 
        //     espl.type='pay' AND
        //     espl.product_id = 8 AND 
        //     espl.pay_num > 0
        // GROUP BY espl.uid
        // ORDER BY sum_pay_num DESC
        // LIMIT 10;
        $data = [];
        $productId = $request->postMore([
            ['id', '']
        ])['id'];
        $data = Db::query("SELECT espl.id, espl.uid, espl.product_id, SUM(espl.pay_num) AS sum_pay_num, eu.nickname, eu.avatar FROM eb_store_product_log espl LEFT JOIN eb_user eu ON eu.uid = espl.uid WHERE espl.type='pay' AND espl.product_id = ? AND espl.pay_num > 0 GROUP BY espl.uid ORDER BY sum_pay_num DESC LIMIT 10;", [$productId]);
        return app('json')->success($data);
    }
